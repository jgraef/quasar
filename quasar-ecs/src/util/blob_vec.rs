...
///
/// Used to densely store homogeneous ECS data. A blob is usually just an
/// arbitrary block of contiguous memory without any identity, and could be used
/// to represent any arbitrary data (i.e. string, arrays, etc). This type is an
/// extendable and re-allocatable blob, which makes it a blobby Vec, a
/// `BlobVec`.
pub struct BlobVec {
    item_layout: Layout,
    capacity: usize,
...
impl BlobVec {
    ...
    ///
    /// `drop` is an optional function pointer that is meant to be invoked when
    /// any element in the [`BlobVec`] should be dropped. For all Rust-based
    /// types, this should match 1:1 with the implementation of [`Drop`]
    /// if present, and should be `None` if `T: !Drop`. For non-Rust based
    /// types, this should match any cleanup processes typically associated
    /// with the stored element.
    ///
    /// # Safety
    ///
    /// `drop` should be safe to call with an [`OwningPtr`] pointing to any item
    /// that's been pushed into this [`BlobVec`].
    ///
    /// If `drop` is `None`, the items will be leaked. This should generally be
    /// set as None based on [`needs_drop`].
    ///
    ...
    /// be inserted in the given `BlobVec`. After calling `reserve_exact`,
    ...
    /// Reserves the minimum capacity for at least `additional` more elements to
    /// be inserted in the given `BlobVec`.
    #[inline]
    pub fn reserve(&mut self, additional: usize) {
        /// Similar to `reserve_exact`. This method ensures that the capacity
        /// will grow at least `self.capacity()` if there is no
    ...
    fn grow_exact(&mut self, increment: NonZero<usize>) {
        ...
        let new_data = if self.capacity == 0 {
            ...
            // - `item_layout.size() > 0` and `new_capacity > 0`, so the layout size is
            //   non-zero
            ...
            // since the item size is always a multiple of its alignment, the rounding
            // cannot happen here and the overflow is handled in `array_layout`
            unsafe {
                alloc::alloc::realloc(
    ...
    /// Initializes the value at `index` to `value`. This function does not do
    /// any bounds checking.
    ///
    /// # Safety
    /// - index must be in bounds
    /// - the memory in the [`BlobVec`] starting at index `index`, of a size
    ///   matching this [`BlobVec`]'s `item_layout`, must have been previously
    ///   allocated.
    #[inline]
    ...
    /// Replaces the value at `index` with `value`. This function does not do
    /// any bounds checking.
    ///
    /// # Safety
    /// - index must be in-bounds
    /// - the memory in the [`BlobVec`] starting at index `index`, of a size
    ///   matching this [`BlobVec`]'s `item_layout`, must have been previously
    ///   initialized with an item matching this [`BlobVec`]'s `item_layout`
    /// - the memory at `*value` must also be previously initialized with an
    ///   item matching this [`BlobVec`]'s `item_layout`
    pub unsafe fn replace_unchecked(&mut self, index: usize, value: OwningPtr<'_>) {
        ...
        if let Some(drop) = self.drop {
            ...
            // SAFETY:
            // - `destination` was obtained from a `PtrMut` in this vector, which ensures it
            //   is non-null, well-aligned for the underlying type, and has proper
            //   provenance.
            ...
            // Make the vector's contents observable again, since panics are no longer
            // possible.
            self.len = old_len;
        }
        ...
        // SAFETY:
        // - `source` and `destination` were obtained from `OwningPtr`s, which ensures
        //   they are valid for both reads and writes.
        // - The value behind `source` will only be dropped if the above branch panics,
        //   so it must still be initialized and it is safe to transfer ownership into
        //   the vector.
        // - `source` and `destination` were obtained from different memory locations,
        //   both of which we have exclusive access to, so they are guaranteed not to
        //   overlap.
        unsafe {
    ...
    ///
    /// # Safety
    /// The `value` must match the [`layout`](`BlobVec::layout`) of the elements
    /// in the [`BlobVec`].
    #[inline]
    ...
    ///
    /// # Safety
    /// It is the caller's responsibility to ensure that `index` is less than
    /// `self.len()`.
    ...
    pub unsafe fn swap_remove_and_forget_unchecked(&mut self, index: usize) -> OwningPtr<'_> {
        debug_assert!(index < self.len());
        // Since `index` must be strictly less than `self.len` and `index` is at least
        // zero, `self.len` must be at least one. Thus, this cannot underflow.
        ...
        self.len = new_len;
        // Cannot use get_unchecked here as this is technically out of bounds after
        // changing len. SAFETY:
        // - `new_len` is less than the old len, so it must fit in this vector's
        //   allocation.
        // - `size` is a multiple of the erased type's alignment, so adding a multiple
        //   of `size` will preserve alignment.
        // - The removed element lives as long as this vector's mutable reference.
        let p = unsafe { self.get_ptr_mut().byte_add(new_len * size) };
        // SAFETY: The removed element is unreachable by this vector so it's safe to
        // promote the `PtrMut` to an `OwningPtr`.
        unsafe { p.promote() }
    }
    ...
    ///
    /// # Safety
    /// It is the caller's responsibility to ensure that `index` is `<
    /// self.len()`.
    #[inline]
    ...
    pub unsafe fn get_unchecked(&self, index: usize) -> Ptr<'_> {
        ...
        let size = self.item_layout.size();
        // SAFETY:
        // - The caller ensures that `index` fits in this vector, so this operation will
        //   not overflow the original allocation.
        // - `size` is a multiple of the erased type's alignment, so adding a multiple
        //   of `size` will preserve alignment.
    ...
    /// Returns a mutable reference to the element at `index`, without doing
    /// bounds checking.
    ///
    /// # Safety
    /// It is the caller's responsibility to ensure that `index < self.len()`.
    ...
    /// Get a reference to the entire [`BlobVec`] as if it were an array with
    /// elements of type `T`
    ///
    /// # Safety
    ...
    pub fn clear(&mut self) {
        ...
        if let Some(drop) = self.drop {
            ...
            for i in 0..len {
                ...
                // * The item is left unreachable so it can be safely promoted to an
                //   `OwningPtr`.
                // NOTE: `self.get_unchecked_mut(i)` cannot be used here, since the
                // `debug_assert` would panic due to `self.len` being set to 0.
                let item = unsafe { self.get_ptr_mut().byte_add(i * size).promote() };
                // SAFETY: `item` was obtained from this `BlobVec`, so its underlying type must
                // match `drop`.
                unsafe { drop(item) };
            }
        }
...
/// - The resulting [`Layout`] is valid, by ensuring that `(layout.size() +
///   padding_needed_for(layout, layout.align())) * n` doesn't overflow.
...
mod tests {
    ...
    /// # Safety
    ///
    /// The pointer `x` must point to a valid value of type `T` and it must be
    /// safe to drop this value.
    ...
    fn blob_vec() {
        let drop_counter = Rc::new(RefCell::new(0));
        {
            ...
            // SAFETY: the following code only deals with values of type `Foo`, which
            // satisfies the safety requirement of `push`, `get_mut` and `swap_remove` that
            // the values have a layout compatible to the blob vec's
            // `item_layout`. Every index is in range.
            unsafe {
                let foo1 = Foo {
                    a: 42,
                    b: "abc".to_string(),
    ...
    fn blob_vec_zst_size_overflow() {
        ...
        // SAFETY: Because `()` is a ZST trivial drop type, and because `BlobVec`
        // capacity   is always `usize::MAX` for ZSTs, we can arbitrarily set
        // the length   and still be sound.
